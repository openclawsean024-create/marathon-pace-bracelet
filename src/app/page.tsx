      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white rounded-2xl shadow-xl p-6 space-y-6">
            ...
            <div className="space-y-2">
              {Object.values(COURSES).map((c) => (
                <button key={c.id} onClick={() => setCourseId(c.id)} className={`w-full text-left p-3 rounded-xl border-2 ${courseId === c.id ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200'}`}>
                  ...
                  <div className="text-xs opacity-70">D+ {c.totalDPlus}m · {c.totalKm}K</div>
                </button>
              ))}
            </div>
            <div className="space-y-2">
            ...
            <button onClick={async () => { setIsGenerating(true); try { await generateWristbandPDF({ targetTime, course, strategy, showElevation, showQR }); } finally { setIsGenerating(false); } }} disabled={isGenerating} className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl text-lg">
